...
async function getUserInfo(email?: string): Promise<UserInfo> {
  ...
  const mockUser: UserInfo = {
    ...
    // 可選的自訂行程
    customSchedule: email?.includes('custom') ? {
      // 範例: 週三上午去圖書館
      "2024-10-02": { am: "其他地點:圖書館", pm: "弘道基地" },
      "2024-10-04": { am: "吉林基地", pm: "其他地點:實驗室" }
    } : undefined
  };
...
export async function GET(request: NextRequest) {
  try {
    ...
    const payload = generateWeeklyPayload({
      ...
      weekStartISO,
      userInfo,
      // 可以在這裡從請求中獲取自訂排程，暫時使用 userInfo 中的自訂排程
    });
